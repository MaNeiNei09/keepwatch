...
#### 市场情绪检测
- **恐惧贪婪指数**：基于RSI动态计算，超卖/超买信号提示
- **交易所净流入/流出**：监测资金流向，判断市场抛压
- **永续合约资金费率**：检测多空拥挤程度，预警反转风险

#### 波动与胜率分析
- **ATR波动率评级**：评估市场波动强度，指导杠杆设置
- **历史相似行情胜率**：参考历史数据，提升决策信心

#### 做空专属检测
- **假突破确认**：识别多头陷阱，捕捉假突破后的做空机会
- **流动性枯竭预警**：监测成交量萎缩，预判突破失败
- **宏观利空事件倒计时**：追踪重大利空事件时间点

#### 全局雷达组件
- 新增 `GlobalRadar.vue` 组件
- 扫描市场TOP 20热门交易对
- 实时显示各交易对做多/做空得分
- 信号提示（🚀做多/📉做空/⏸️观望）
- 支持点击快速切换交易对
...
| 层级 | 做多检测点 | 做空检测点 |
|------|-----------|-----------|
| 战略层 | 箱体定位、大级别中枢 | 箱体定位、大级别中枢 |
| 战术层 | 黄金坑位、共振验证 | 黄金压力位、共振验证 |
| 触发层 | 底分型、笔终结、背驰、类二买 | 顶分型、笔终结、背驰、类二卖 |
| 市场情绪层 | 恐惧贪婪指数、交易所净流入/流出、资金费率 | 恐惧贪婪指数、交易所净流入/流出、资金费率 |
| 波动与胜率层 | ATR波动率评级、历史相似行情胜率 | ATR波动率评级、历史相似行情胜率 |
| 参数与风控 | 入场点、止损点、盈亏比、杠杆建议 | 入场点、止损点、盈亏比、杠杆建议 |
| 做空专属 | - | 假突破确认、流动性枯竭预警、宏观利空事件倒计时 |
...
#### 交易检测页面
- 新增「交易检测」页面，融合三维共振交易系统
- **核心逻辑架构**：
  - 🏛️ 箱体 (The Box)：解决"在哪里战" - 识别供需区间，确定支撑与阻力
  - 📐 斐波那契 (Fibonacci)：解决"在哪进场" - 寻找高概率的回撤/反弹位
  - 🔬 缠论 (Chan Theory)：解决"何时开枪" - 精准捕捉转折点

#### 做多检测清单
- **战略层**：箱体定位、大级别中枢判断
- **战术层**：斐波那契黄金坑位(0.5/0.618/0.786)、共振验证
- **触发层**：底分型确认、笔的终结、盘整背驰、类二买信号
- **参数与风控**：入场点、止损点、盈亏比、杠杆建议

#### 做空检测清单
- **战略层**：箱体定位、大级别中枢判断
- **战术层**：斐波那契黄金压力位、共振验证
- **触发层**：顶分型确认、笔的终结、盘整背驰、类二卖信号
- **参数与风控**：入场点、止损点、盈亏比、杠杆建议

#### 决策建议
- 综合评分显示（做多/做空得分）
- 智能决策建议（强烈做多/做空、谨慎做多/做空、观望）
- 大师心法提醒
- 执行纪律规则

### 核心交易心法
- 三缺一无效：箱体+斐波那契+缠论背驰/分型，三者缺一不可
- 宁可错过，不做错：市场不缺机会，缺的是本金
- 突破不看幅度，看结构：无背驰不突破，有背驰必回归
