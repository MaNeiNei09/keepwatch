...
### 交易检测页面 (/checklist)
**三维共振交易系统** - 融合箱体理论、斐波那契、缠论
- **界面布局**：决策建议+全局雷达（左）| 做多检测（中）| 做空检测（右）
- **核心逻辑**：
  - 🏛️ 箱体 (The Box)：解决"在哪里战" - 识别供需区间
  - 📐 斐波那契 (Fibonacci)：解决"在哪进场" - 黄金分割位
  - 🔬 缠论 (Chan Theory)：解决"何时开枪" - 分型/背驰
- **做多检测清单**：
  - 战略层：箱体定位、大级别中枢
  - 战术层：黄金坑位(0.5/0.618/0.786)、共振验证
  - 触发层：底分型、笔终结、盘整背驰、类二买
  - 市场情绪层：恐惧贪婪指数、交易所净流入/流出、资金费率
  - 波动与胜率层：ATR波动率评级、历史相似行情胜率
  - 参数与风控：入场点、止损点、盈亏比、杠杆建议
- **做空检测清单**：
  - 战略层：箱体定位、大级别中枢
  - 战术层：黄金压力位、共振验证
  - 触发层：顶分型、笔终结、盘整背驰、类二卖
  - 市场情绪层：恐惧贪婪指数、交易所净流入/流出、资金费率
  - 波动与胜率层：ATR波动率评级、历史相似行情胜率
  - 参数与风控：入场点、止损点、盈亏比、杠杆建议
  - 做空专属检测：假突破确认、流动性枯竭预警、宏观利空事件倒计时
- **决策建议**：
  - 综合评分显示
  - 智能决策建议
  - 大师心法提醒
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v2.7.0 | 2026-03-23 | 市场情绪检测、波动胜率分析、做空专属检测、全局雷达组件 |
| v2.6.0 | 2026-03-23 | 交易检测页面：三维共振系统（箱体+斐波那契+缠论） |
